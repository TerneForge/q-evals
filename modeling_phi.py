...
from qlinear import QLinear # quantized linear layers for 1.58 bit per-channel training
...
class PhiMLP(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = QLinear(config.hidden_size, config.intermediate_size)
        self.fc2 = QLinear(config.intermediate_size, config.hidden_size)
...
class PhiAttention(nn.Module):
    ...
    def __init__(self, config: PhiConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = QLinear(self.hidden_size, self.num_heads * self.head_dim, bias=True)
        self.k_proj = QLinear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = QLinear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.dense = QLinear(self.num_heads * self.head_dim, self.hidden_size, bias=True)
...
class QPhiForCausalLM(PhiPreTrainedModel):
